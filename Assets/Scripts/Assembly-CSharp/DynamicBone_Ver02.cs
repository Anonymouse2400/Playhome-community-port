...
public class DynamicBone_Ver02 : MonoBehaviour
{
	...
	public bool setSoftParams(int _ptn, int _bone, float _damping, float _elasticity, float _stiffness, bool _isNowParam = true)
	{
		...
		else
		{
			...
			if (_bone == -1)
			{
				for (int l = 0; l < Patterns.Count; l++)
				{
					setSoftParams(Patterns[_ptn].ParticlePtns[l], _damping, _elasticity, _stiffness);
				}
			}
			else if (Particles.Count > _bone)
			{
	...
	private void InitNodeParticle()
	{
		...
		foreach (BonePtn pattern in Patterns)
		{
			...
			{
				Debug.LogWarning("パラメータの数と骨の数があっていない");
				continue;
			}
	...
	private void SkipUpdateParticles()
	{
		foreach (Particle particle2 in Particles)
		{
			if (particle2.ParentIndex >= 0)
			{
				Vector3 vector = ObjectMove * particle2.Inert;
				particle2.PrevPosition += vector;
				particle2.Position += vector;
				Particle particle = Particles[particle2.ParentIndex];
				float num = ((!(particle2.Transform != null)) ? (particle2.EndOffset.magnitude * ObjectScale) : (particle.Transform.position - particle2.Transform.position).magnitude);
				float num2 = Mathf.Lerp(1f, particle2.Stiffness, Weight);
				if (num2 > 0f)
				{
					Matrix4x4 localToWorldMatrix = particle.Transform.localToWorldMatrix;
					localToWorldMatrix.SetColumn(3, particle.Position);
					Vector3 vector2 = ((!(particle2.Transform != null)) ? localToWorldMatrix.MultiplyPoint3x4(particle2.EndOffset) : localToWorldMatrix.MultiplyPoint3x4(particle2.LocalPosition));
					Vector3 vector3 = vector2 - particle2.Position;
					float magnitude = vector3.magnitude;
					float num3 = num * (1f - num2) * 2f;
					if (magnitude > num3)
					{
						particle2.Position += vector3 * ((magnitude - num3) / magnitude);
					}
				}
				Vector3 vector4 = particle.Position - particle2.Position;
				float magnitude2 = vector4.magnitude;
				if (magnitude2 > 0f)
				{
					float num4 = (magnitude2 - num) / magnitude2;
					if (particle2.KeepLengthLimitMin >= num4)
					{
						particle2.Position += vector4 * (num4 - particle2.KeepLengthLimitMin);
					}
					else if (num4 >= particle2.KeepLengthLimitMax)
					{
						particle2.Position += vector4 * (num4 - particle2.KeepLengthLimitMax);
					}
				}
			}
			else
			{
				particle2.PrevPosition = particle2.Position;
				particle2.Position = particle2.Transform.position;
			}
		}
	}
	...
	private void ApplyParticlesToTransforms()
	{
		for (int i = 1; i < Particles.Count; i++)
		{
			...
			if (particle.IsCrush)
			{
				...
				if (v.z <= 0f)
				{
					float num2 = Mathf.Clamp01(Mathf.InverseLerp(particle.CrushMoveAreaMin, 0f, v.z));
	...
	private bool LoadText(LoadInfo _info, List<string> _list, ref int _index)
	{
		...
		case "#ReflectSpeed":
		{
			...
			{
				Debug.LogWarning("ReflectSpeedの数値取得に失敗");
				return false;
			}
		...
		case "#HeavyLoopMaxCount":
		{
			...
			{
				Debug.LogWarning("HeavyLoopMaxCountの数値取得に失敗");
				return false;
			}
		...
		case "#Colliders name":
		{
			for (int i = 1; i < num && !(array[i] == string.Empty) && !(array[i] == " "); i++)
			{
				...
				if (transform == null)
				{
					Debug.LogWarning("[Colliders name] " + array[i] + " この名前のフレームは見つかりません");
					return false;
				}
				DynamicBoneCollider component = transform.GetComponent<DynamicBoneCollider>();
				if (component == null)
				{
					Debug.LogWarning("[Colliders name] " + array[i] + " このフレームにDynamicBoneColliderコンポーネントは付いていません");
					return false;
				}
		...
		case "#Bone name":
		{
			for (int j = 1; j < num && !(array[j] == string.Empty) && !(array[j] == " "); j++)
			{
				...
				if (transform2 == null)
				{
					Debug.LogWarning("[Bone name] " + array[j] + " この名前のフレームは見つかりません");
					return false;
				}
		...
		default:
			Debug.LogWarning("テキスト：識別不能 " + array[0] + " [index]:" + _index);
			return false;
		}
		_index++;
		return true;
	...
	private bool LoadParamClassMember(BonePtn _ptn, List<string> _list, ref int _index)
	{
		while (_list.Count > _index)
		{
			...
			if (num <= _index2)
			{
				Debug.LogWarning("[ParamClassMember] 行に空欄があります？");
				return false;
			}
			...
			if (transform == null)
			{
				Debug.LogWarning("[ParamClassMember] " + array[_index2] + " この名前のフレームは見つかりません");
				return false;
			}
	...
	private bool ChekcLength(int _length, ref int _index, int _line, string _warning = "", string _warning1 = "")
	{
		if (_length <= ++_index)
		{
			Debug.LogWarning(_warning + "までの設定数が足りない [" + _line + "行目] " + _warning1);
			return false;
		}
		return true;
	}
	...
	private bool GetMemberFloat(int _length, string[] _str, ref int _index, int _line, out float _param, string _warning = "", string _warning1 = "")
	{
		...
		{
			Debug.LogWarning(_warning + "が取得できない [" + _line + "行目] " + _str[_index] + _warning1);
			return false;
		}
		return true;
	}
