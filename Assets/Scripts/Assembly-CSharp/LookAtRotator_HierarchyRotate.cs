...
public class LookAtRotator_HierarchyRotate : MonoBehaviour
{
	...
	public void Calc()
	{
		...
		Vector3 localEulerAngles = baseBone.transform.localEulerAngles;
		...
		foreach (RotateBone rotateBone in array)
		{
			...
			else if (calcType == TYPE.TARGET)
			{
				...
				rel *= turnRate;
				rel.y = Mathf.Clamp(rel.y, 0f - rotateBone.maxLeft, rotateBone.maxRight);
			...
			else if (calcType == TYPE.AWAY)
			{
				...
				rel.y = ((!(zero.y > 0f)) ? rotateBone.maxRight : (0f - rotateBone.maxLeft));
				rel.x = ((!(zero.x > 0f)) ? maxDown : (0f - maxUp));
				rel.z = 0f;
			}
			rotateBone.Interpolation(rel, turnPower, turnMaxSpeed, turnMinSpeed);
			rotateBone.PrevEuler = rotateBone.Euler;
			Quaternion quaternion = Quaternion.Euler(rotateBone.Euler);
